...
- **B056** — `flagged_when` Semantics Incompatible with `transform` Data Operations. url-canonicalizer's manifest declares `flagged_when: { "status": ["redirected"] }` which causes the skeleton's approve route to silently drop the URLs canonicalizer actually transformed (11 of 152 in observed Pronet Gaming run). The skeleton applies `flagged_when` semantics identically for `remove` and `transform` operations, but the semantics only make sense for `remove`. Phase 3 introduces new submodules; manifest authors will copy-paste the same trap. Fix Option A (modules-v2 manifest change, ~1 line) before Phase 3 starts; document the `flagged_when` rule for `transform` ops in modules-v2 CLAUDE.md Rule 12 as part of B055's audit output. Filed 2026-06-06. See `specs/BACKLOG.md` for full diagnosis.
- **B058** — content-analyzer + content-writer manifest defaults still hardcode iGaming/B2B/OnlyiGaming framing. seo-planner v2.1.0 + tone-seo-editor v1.2.0 (both 2026-06-07) hold the May 22 commitment that OnlyiGaming-specific prompts belong in template preset overrides, not module defaults. Bring the remaining two Step 5 modules to the same pipeline-agnostic standard before Phase 3 starts — manifest authors will otherwise copy the iGaming-locked pattern when writing qa-structural, humanizer, qa-ai-detector, news/podcast generation modules. Filed 2026-06-07. See `specs/BACKLOG.md` for full diagnosis.
- **B059** — seo-planner's CRITICAL RULE ("keywords MUST come from research") becomes paradoxical when `keyword_research: false` disables Perplexity. The v2.1.0 fallback instruction handles non-silent failure, but the paradoxical rule itself remains in the prompt. `buildPrompt()` should conditionally strip or rewrite the CRITICAL RULE block when research is empty (e.g., via a `<!-- only-when-research -->` marker pair pattern). Phase 3 introduces new prompts with similar integrity rules (qa-structural, humanizer, qa-ai-detector); the conditional-section pattern should ship before Phase 3 authors inherit the same paradox. Filed 2026-06-07. See `specs/BACKLOG.md` for full diagnosis.
- **B062** — Per-provider model selector pattern needed for future search providers (Gemini, Ahrefs, Semrush mentioned in seo-planner's `search_provider` description as future options). Speculative until any second provider ships; trigger to file as concrete work is the first non-Perplexity provider integration. Filed 2026-06-07. See `specs/BACKLOG.md` for full diagnosis.
