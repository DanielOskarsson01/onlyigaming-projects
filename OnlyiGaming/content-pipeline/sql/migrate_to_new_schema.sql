-- Content-Pipeline Schema Migration
-- Handles existing tables from old schema
...
-- ============================================
-- STEP 2: Rename conflicting old tables
-- ============================================
-- Only run these if the tables exist and you want to preserve them

-- Old schema tables that might conflict:
ALTER TABLE IF EXISTS content_items RENAME TO _legacy_content_items;
ALTER TABLE IF EXISTS platform_tags RENAME TO _legacy_platform_tags;
ALTER TABLE IF EXISTS content_tags RENAME TO _legacy_content_tags;
ALTER TABLE IF EXISTS pipeline_stages RENAME TO _legacy_pipeline_stages;
ALTER TABLE IF EXISTS pipeline_runs RENAME TO _legacy_pipeline_runs;
ALTER TABLE IF EXISTS projects RENAME TO _legacy_projects;
ALTER TABLE IF EXISTS discovered_urls RENAME TO _legacy_discovered_urls;
ALTER TABLE IF EXISTS scraped_pages RENAME TO _legacy_scraped_pages;
ALTER TABLE IF EXISTS generated_content RENAME TO _legacy_generated_content;

-- ============================================
-- STEP 3: Disable old pg_cron job (if exists)
-- ============================================
-- SELECT cron.unschedule('purge-filtered-content-body');

-- ============================================
-- STEP 4: Create new tables (fresh)
-- ============================================
